...
import { updateState, setActiveChannel, createNewConversation } from '../managers/cache-manager';
import { loadModel } from '../api-connections/model-loader';
...
export const handleModelCommand = async (interaction: CommandInteraction<CacheType>, modelDirectories: ModelDirectories) => {
  ...
  if (modelName in modelDirectories) {
    ...
    try {
      ...
      await loadModel(modelName, interaction);
    } catch (error) {
