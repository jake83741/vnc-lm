import { EmbedBuilder, TextChannel, ThreadChannel, ChannelType, ThreadAutoArchiveDuration } from 'discord.js';
...
import { updateState, setActiveChannel, createNewConversation, chatBot, cacheStore, BotSettings, adminUserId, defaultNumCtx, Conversation, CachedMessageData } from '../utilities';
...
export class ModelManager {
    ...
    public async loadModelDirectories(): Promise<ExtendedModelDirectories> {
        ...
        try {
            const response = await axios.get('http://host.docker.internal:11434/api/tags', { timeout: 5000 });
    ...
    private handleModelError(error: any, modelName: string): never {
        if (axios.isAxiosError(error)) {
            ...
            if (error.response?.status === 429) {
                ...
                throw new Error(`Rate limit exceeded for model ${modelName}. Please wait ${waitTime} seconds.`);
            }
            throw new Error(`LiteLLM error: ${errorData?.error?.message || errorData?.message || error.message}`);
        }
...
export class ModelCommand extends BaseCommand {
    ...
    async execute(context: CommandContext) {
        ...
        const params: ModelCommandParams = {
            ...
            systemPrompt: interaction.options.get('system_prompt')?.value as string | undefined,
            temperature: interaction.options.get('temperature')?.value as number | undefined,
    ...
    private async handleModelLoad(context: CommandContext, params: ModelCommandParams) {
        const { interaction } = context;
        if (!interaction) return;
        ...
        try {
            ...
            if (interaction.channel?.type === ChannelType.GuildText) {
                const threadName = generateThreadName(params.systemPrompt || "New Conversation");
    ...
    private async handleModelRemoval(context: CommandContext, modelName: string) {
        if (!await this.checkPermissions(context, adminUserId)) {
            await this.sendResponse(context, 'You do not have permission to remove models.', true);
            return;
        }
        ...
        try {
            ...
            if (chatBot.modelName === modelName) {
                ...
                if (context.interaction?.client.user) {
                    context.interaction.client.user.setActivity('no active model, use /model');
                }
                updateState({
                    lastUsedModel: null,
            ...
            await this.sendResponse(context, `The model ${modelName} has been removed.`, true);
        } catch (error) {
            await this.sendResponse(context, `Failed to remove model ${modelName}.`, true);
        }
    }
}
...
// Helper Functions
export function generateThreadName(message: string, cachedContent?: string): string {
    try {
        const contentToUse = message || "New Conversation";
        
        const keywords = keyword_extractor.extract(contentToUse, {
            language: "english",
            remove_digits: true,
        ...
        const firstFewWords = contentToUse
            .split(/\s+/)
            .slice(0, 3)
            .join("-")
            .toLowerCase()
        ...
        if (firstFewWords && firstFewWords.length > 0) {
            return firstFewWords.length > 100 ? firstFewWords.slice(0, 97) + "..." : firstFewWords;
        }
...
export function generateTreeDiagram(parentName: string, currentName: string): string {
    return [
        '```',
        `├── ${parentName}`,
        `│   └── ${currentName}`,
...
export function buildHelpEmbed(): EmbedBuilder {
    return new EmbedBuilder()
        ...
        .setDescription('This is a Discord bot for using local and hosted language models.')
        .addFields([
            {
                name: 'Model Command',
                value: '`/model [model] [num_ctx] [system_prompt] [temperature] [remove]`'
            },
            {
                name: 'Parameters',
                value: [
...
export class CommandError extends Error {
    constructor(
        message: string,
        public readonly type: 'PERMISSION' | 'VALIDATION' | 'EXECUTION' = 'EXECUTION'
    ) {
        super(message);
...
export const validators = {
    ...
    isValidContextSize: (size?: number | null): boolean => {
        return size === null || size === undefined || (size >= 512 && size <= 32768);
    }
};
