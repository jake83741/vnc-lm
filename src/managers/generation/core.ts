import { Message, MessageType, TextChannel, DMChannel, NewsChannel, ThreadChannel, Client, Interaction, Attachment, PartialMessage } from 'discord.js';
import axios from 'axios';
import sharp from 'sharp';
import { chatBot, adminUserId, requireMention, MessageData, Conversation, CachedMessageData, cacheStore, updateState, getState, updateMessageCache, getActiveChannel, setActiveChannel } from '../../utilities';
...
async function preprocessMessage(message: Message): Promise<PreprocessedMessage> {
    ...
    if (modelSwitch) {
        try {
            ...
            newClient.setSystem(threadSystemPrompt);
...
export async function handleMessageCreate(message: Message) {
    ...
    if (message.reference && message.content.toLowerCase() === 'branch') {
        await threadCommand.execute({ message, isThread: message.channel.isThread() });
        return;
    }
    ...
    try {
        ...
        if (processed.isModelSwitch || await stopCommand.execute({ message, isThread: message.channel.isThread() })) return;
    
        if (processed.userInput) {
            if (message.channel.isThread()) {
                const threadChannel = message.channel as ThreadChannel;
                if (threadChannel.name === "new-thread" || threadChannel.name === "conversation") {
...
export async function findClosestModelMatch(input: string): Promise<string | null> {
    try {
