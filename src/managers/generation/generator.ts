...
import { addToPages, createPageEmbed, createPageButtons } from "./formatter";
    ...
    try {
      if (!responseMessage) {
        ...
        await responseMessage.edit(
          messageOptions as MessagePayload | MessageEditOptions
        );
      }
    } catch (error: any) {
...
export async function handleMessageResponse(
  ...
  const currentClient = chatBot.modelName
    ? ClientFactory.getClient(chatBot.modelName)
    : null;
  if (!currentClient) {
    await message.reply(
      "No active model. Please set a model using the /model command."
    );
    return;
  }
