...
function findOptimalSplitPoint(content: string, limit: number): number {
    ...
    if (splitPoint === -1) {
        splitPoint = findMarkdownBoundaries(content, lastNewline > 0 ? lastNewline + 1 : limit);
    }
...
function updateMarkdownState(content: string, state: MarkdownState): void {
    ...
    state.inlineCode = (matches.inlineCode.length % 2) !== 0 ? !state.inlineCode : state.inlineCode;
}
...
export function addToPages(messageData: MessageData, content: string): void {
    ...
    while (content) {
        ...
        while (/^[\*_`]*\d+\.$/.test(content.slice(Math.max(0, splitPoint - 5), splitPoint + 1))) {
        ...
        if (lastPageIndex === messageData.pages.length - 1 && /\*$/.test(pageContent)) {
            const lastAsteriskIndex = pageContent.lastIndexOf('*');
            pageContent = pageContent.slice(0, lastAsteriskIndex) + pageContent.slice(lastAsteriskIndex + 1);
        }
