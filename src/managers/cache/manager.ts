...
import { cacheStore, getState } from "./store";
import { setupGlobalMessageCollector } from "../generation/core";
import { chatBot } from "../../utilities";
import { ClientFactory } from "../../api-connections/factory";
import { clientInstance } from "../../bot";
...
class CacheManagerImpl {
  ...
  public async restoreMessages(client: Client): Promise<void> {
    ...
    const activeMessages = conversations.flatMap(([_, conv]) => {
      ...
      return typedConv.messages.filter(
  ...
  public getConversations(): { [id: string]: Conversation } {
    ...
    return Object.entries(cacheStore.cache.conversations).reduce(
      (filtered, [id, conv]) => {
        const typedConv = conv as Conversation;
        const messages = typedConv.messages.filter(
  ...
  public createConversation(systemPrompt?: string | null): string {
    const state = cacheStore.getState();
    const conversationId = `conv-${(++state.conversationCounter)
      .toString()
      .padStart(4, "0")}`;
  ...
  public updateMessage(
    ...
    if (!activeChannelId || !currentConversationId) return;
  
    const conversation = cacheStore.cache.conversations[currentConversationId];
    if (!conversation) return;
  
    // Check if this is the first message or matches the conversation's channel
    const conversationChannelId = conversation.messages[0]?.channelId;
    if (conversationChannelId && channelId !== conversationChannelId) return;
  ...
  public deleteMessage(messageId: string, channelId: string): void {
    ...
    const messageIndex = conversation.messages.findIndex(
      (msg) => msg.messageId === messageId && msg.channelId === channelId
    );
  ...
  public getActiveChannel(): Channel | null {
    ...
    const channel = clientInstance.channels.cache.get(activeChannelId);
    return channel || null;
  }
...
export const restoreMessageDataFromCache = async (
  client: Client
): Promise<void> => await cacheManager.restoreMessages(client);
...
export const deleteMessageFromCache = (
  messageId: string,
  channelId: string
): void => cacheManager.deleteMessage(messageId, channelId);
export const getActiveChannel = (): Channel | null =>
  cacheManager.getActiveChannel();
export const setActiveChannel = async (
  channel: Channel | null
): Promise<void> => await cacheManager.setActiveChannel(channel);
