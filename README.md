...
### A Discord bot for large language models. Add Gemini 2.5 Pro, Claude Sonnet 3.7, GPT 4.1, and other models.
...
After being enabled with `+ search`, the bot will use a RAG pipeline to gather additional context from multiple sources: [Wikipedia](https://www.wikipedia.org/) articles, web search results, and news sources. The bot will pull the most relevant content chunks and add around 300 tokens of context to the user query, delivering highly targeted information while maintaining efficiency. This context represents just a small percentage of the total information gathered during search. Web search can be turned off with `+ model`.
