...
/model model:command-a:111b-03-2025-q8_0 num_ctx:32000
# downloading an ollama model by sending a model tag link
https://ollama.com/library/command-a:111b-03-2025-q8_0
# removing an ollama model
/model model:command-a:111b-03-2025-q8_0 remove:True
```
...
After being enabled with `+ search`, the bot will use a RAG pipeline to gather additional context from [Wikipedia](https://www.wikipedia.org) and [DuckDuckGo](https://duckduckgo.com). The context gathered will then be passed to a vector store and analyzed for relevance using a hybrid approach that combines vector similarity and keyword matching. The bot will then pull the most relevant chunks of context and add it to the user query `~500 tokens`, a small percentage of the total context gathered. Web search can be turned off with `+ model`. 
