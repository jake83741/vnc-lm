...
After being enabled with `+ search`, the bot will use a RAG pipeline to gather additional context from [Wikipedia](https://www.wikipedia.org) and [DuckDuckGo](https://duckduckgo.com). The context gathered will then be passed to a vector store and analyzed for relevance using a hybrid approach that combines vector similarity and keyword matching. The bot will then pull the most relevant chunks of context and add `~500 tokens` to the user query, a small percentage of the total context gathered. Web search can be turned off with `+ model`. 
